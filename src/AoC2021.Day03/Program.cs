    ...
    public static class Program
    {
        ...
        internal static int Day03_Part_2(int[][] input)
        {
            return _convertOutput(
                ProcessDiagnostics(input, true, 0)[0]) * _convertOutput(ProcessDiagnostics(input, false, 0)[0]);
        }

        internal static int[][] ProcessDiagnostics (int[][] input, bool mostCommon, int i)
        {
            int bit = input.Count(d => d[i] == 1) >= (input.Length / 2) ? 1 : 0;

            input = input.Where(d => d[i] == (mostCommon ? bit : (bit == 1 ? 0 : 1))).ToArray();

            return input.Length > 1 ? ProcessDiagnostics(input, mostCommon, i + 1) : input;
        }
